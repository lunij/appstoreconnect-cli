...
struct AuthOptions: ParsableArguments {
    ...
    enum Error: Swift.Error, CustomStringConvertible, Equatable {
        case apiIssuerIdNotProvided
        case apiKeyIdNotProvided

        var description: String {
            switch self {
            case .apiIssuerIdNotProvided:
                return "App Store Connect API Issuer ID not provided. Either add it as option on command line or use the environment variable APP_STORE_CONNECT_ISSUER_ID."
            case .apiKeyIdNotProvided:
                return "App Store Connect API Key ID not provided. Either add it as option on command line or use the environment variable APP_STORE_CONNECT_API_KEY_ID."
            }
        }
    }
    ...
    @Option(
        help: ArgumentHelp(
            ...
            discussion:
                ...
                If not specified on the command line this value will be read from the environment variable APP_STORE_CONNECT_ISSUER_ID.
                """,
            valueName: "uuid"
        )
    )
    var apiIssuer: IssuerID = .environment("APP_STORE_CONNECT_ISSUER_ID")
    ...
    @Option(
        help: ArgumentHelp(
            ...
            discussion:
                ...
                If not specified on the command line the value of this option will be read from the environment variable APP_STORE_CONNECT_API_KEY_ID.
    ...
    var apiKeyId: APIKeyID = .environment("APP_STORE_CONNECT_API_KEY_ID")
}
