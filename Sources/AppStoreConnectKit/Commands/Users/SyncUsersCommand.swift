...
fileprivate typealias UserChange = CollectionDifference<User>.Change
...
private extension BagbutikService {
    func usersInAppStoreConnect() async throws -> [Model.User] {
    ...
    func sync(changes: CollectionDifference<User>) async throws {
        for change in changes {
            ...
            case let .remove(_, user, _):
                ...
                _ = try await request(.deleteUserV1(id: userId))
            }
        }
    }
