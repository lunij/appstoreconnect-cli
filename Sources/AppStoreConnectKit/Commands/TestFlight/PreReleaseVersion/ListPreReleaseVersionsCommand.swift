...
struct ListPreReleaseVersionsCommand: CommonParsableCommand {
    ...
    @Option(
        ...
        help: ArgumentHelp(
            "Filter by platform: \(ListPreReleaseVersionsV1.Filter.Platform.allValueStringsFormatted)",
            valueName: "platform"
        )
    )
    ...
    func run() async throws {
        ...
        .execute()
        .map(Model.PreReleaseVersion.init)
        .render(options: common.outputOptions)
    }
}

extension ListPreReleaseVersionsV1.Filter.Platform: ExpressibleByArgument {
