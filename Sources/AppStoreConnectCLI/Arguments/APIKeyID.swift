...
extension APIKeyID {
    enum APIKeyError: Swift.Error, CustomStringConvertible, Equatable {
        ...
        var description: String {
            ...
            case .firstLineMissing:
                ...
                The first line of the key is required to contain '\(String.firstLine)'.
                """
            case .lastLineMissing:
                return """
                API Key has invalid format

                The last line of the key is required to contain '\(String.lastLine)'.
                """
            case .linesMissing:
                return """
                API Key has invalid format
    ...
    func loadKey() throws -> String {
        ...
        let lines = apiKey.components(separatedBy: .newlines).filter(\.isNotEmpty)

        guard let firstLine = lines.first, firstLine.contains(String.firstLine) else {
            throw APIKeyError.firstLineMissing
        }

        guard let lastLine = lines.last, lastLine.contains(String.lastLine) else {
